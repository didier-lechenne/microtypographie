 ...
 * @returns Texte transformé
 */
export function processTextWithBlocks(
  text: string,
  blocks: string[],
  rules: CompiledRules
): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentBlockType === "normal") {
      ...
      if (normalTextBuffer.length > 0) {
        const processedBuffer = applyAllRules(
